...
function SuiviDemande({ demandeId }) {
  ...
  useEffect(() => {
    fetch(`http://localhost:4000/api/suivi?demande_id=${demandeId}`)
      .then(res => res.json())
      .then(data => {
        setSuivi(data);
        setLoading(false);
          ...
          {suivi.map((item, idx) => (
            <li key={idx} className="suivi-item">
              <div>
                ...
                {' '}{t('suiviDemande.by')} <span style={{color:'#7fa22b'}}>{item.agent_nom || t('suiviDemande.system')}</span>
                <span style={{marginLeft:8, fontSize:12, color:'#888'}}>
                  {new Date(item.created_at).toLocaleString()}
                </span>
              </div>
              {item.message && <div style={{fontStyle:'italic', color:'#555'}}>{item.message}</div>}
            </li>
          ))}
        </ul>
      )}
