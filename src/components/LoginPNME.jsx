    ...
    <>
      ...
      <div className="login-container">
        <div className="login-intro">
          ...
          <Title level={3} style={{ color: '#229954' }}>{t('loginPNME.title')}</Title>
        </div>
        <div className="login-antd-form-container">
              ...
              rules={[
                ...
                { 
                  validator: (_, value) => {
                    ...
                    if (!value) {
                      return Promise.reject(new Error(t('loginPNME.error_identifiant')));
                    }
                    ...
                    return Promise.reject(new Error(t('loginPNME.error_identifiant_invalid')));
                  }
                }
              ]}
            >
              <Input prefix={<UserOutlined />} placeholder={t('loginPNME.identifiant_placeholder')} />
            </Form.Item>
            <Form.Item
              label={t('loginPNME.password')}
              name="password"
              rules={[{ required: true, message: t('loginPNME.error_password') }]}
            >
              <Input.Password prefix={<LockOutlined />} placeholder={t('loginPNME.password')} />
            </Form.Item>
            <Form.Item>
          ...
          <div className="login-antd-links">
            <a href="/forgot-password" className="login-antd-link">{t('loginPNME.forgot_password')}</a>
          </div>
          <a href="/" className="login-antd-back-home">{t('loginPNME.back_home')}</a>
        </div>
    </div>
      <Footer />
    </>
