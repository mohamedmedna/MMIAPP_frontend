...
const GestionSignatures = ({ user }) => {
  ...
  const fetchSignatures = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE}/api/ministere/signatures`, {
  ...
  const handleUpload = async (values) => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE}/api/ministere/signatures/upload`,
        {
          method: "POST",
          headers: { Authorization: `Bearer ${token}` },
          body: formData,
      ...
      if (response.ok) {
        ...
        message.error(
          errorData.error || t("gestionSignatures.errors.uploadError")
        );
      }
    } catch (error) {
  ...
  const handleDelete = async (signatureId) => {
    try {
      const token =
        localStorage.getItem("adminToken") || localStorage.getItem("token");
      const response = await fetch(
        `${API_BASE}/api/ministere/signatures/${signatureId}`,
        {
          method: "DELETE",
          headers: { Authorization: `Bearer ${token}` },
        }
      ...
      if (response.ok) {
        ...
        const errorData = await response.json();
        message.error(
          errorData.error || t("gestionSignatures.errors.deleteError")
        );
      }
    } catch (error) {
  ...
  const formatFileSize = (bytes) => {
    ...
    const k = 1024;
    const sizes = [
      t("gestionSignatures.fileSize.bytes"),
      t("gestionSignatures.fileSize.kb"),
      t("gestionSignatures.fileSize.mb"),
      t("gestionSignatures.fileSize.gb"),
    ];
  ...
  return (
    <div className="gestion-signatures">
      ...
      <Text type="secondary">{t("gestionSignatures.description")}</Text>
      ...
      <Card title={t("gestionSignatures.mySignatures")} loading={loading}>
        {signatures.length === 0 ? (
          <div style={{ textAlign: "center", padding: "40px 0" }}>
            ...
            />
            <Text type="secondary">
              {t("gestionSignatures.empty.noSignatures")}
            </Text>
            <br />
            <Text type="secondary">
              {t("gestionSignatures.empty.startUpload")}
            </Text>
          </div>
        ) : (
          <List
              ...
              <List.Item
                actions={[
                  <Button
                    ...
                    onClick={() =>
                      window.open(
                        `${API_BASE}/uploads/${signature.fichier_signature}`,
                        "_blank"
                      )
                    }
                  >
                ...
                <List.Item.Meta
                  ...
                  description={
                    <Space direction="vertical" size="small">
                      <Text type="secondary">
                        {t("gestionSignatures.details.type")}:{" "}
                        {signature.type_signature}
                      </Text>
                      <Text type="secondary">
                        {t("gestionSignatures.details.size")}:{" "}
                        {formatFileSize(signature.taille_fichier)}
                      </Text>
                      <Text type="secondary">
                        {t("gestionSignatures.details.uploadDate")}:{" "}
                        {new Date(signature.date_creation).toLocaleDateString(
                          "fr-FR"
                        )}
                      </Text>
                      {signature.commentaire && (
                        <Text type="secondary">
                          {t("gestionSignatures.details.comment")}:{" "}
                          {signature.commentaire}
                        </Text>
                      )}
                    </Space>
        ...
        <Form form={uploadForm} layout="vertical" onFinish={handleUpload}>
          <Form.Item
            ...
            rules={[
              {
                required: true,
                message: t("gestionSignatures.form.typeRequired"),
              },
            ]}
          >
            <Select placeholder={t("gestionSignatures.form.typePlaceholder")}>
              <Option value="AUTORISATION">
                {t("gestionSignatures.types.autorisation")}
              </Option>
              <Option value="ACCUSE">
                {t("gestionSignatures.types.accuse")}
              </Option>
              <Option value="DOCUMENT_OFFICIEL">
                {t("gestionSignatures.types.documentOfficiel")}
              </Option>
            </Select>
          </Form.Item>
          ...
          <Form.Item
            ...
            rules={[
              {
                required: true,
                message: t("gestionSignatures.form.fileRequired"),
              },
            ]}
          >
            <input
          ...
          <Form.Item>
            <Space>
              ...
              >
                {uploading
                  ? t("gestionSignatures.form.uploading")
                  : t("gestionSignatures.form.uploadButton")}
              </Button>
