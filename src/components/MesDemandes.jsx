  ...
  return (
    <div className="barre-progression">
      {ETAPES.map((etape, idx) => (
        <div key={etape} className={`etape ${idx <= indexActuel ? 'active' : ''}`}>
              ...
              <tbody>
                {demandes.map(demande => (
                  <tr key={demande.id}>
                    ...
                    <td>{new Date(demande.dateDepot || demande.created_at).toLocaleDateString()}</td>
                    <td>{demande.type}</td>
                    <td>
                      <span className={`statut-badge statut-${demande.statut.toLowerCase().replace(/\s/g, '-')}`}>
                        {t(`mesDemandes.status.${demande.statut}`) || demande.statut}
                      </span>
                    </td>
                    <td>
                      <BarreProgression statut={t(`mesDemandes.status.${demande.statut}`) || demande.statut} />
                    </td>
                    <td>
                      <button
                        className="btn-consulter"
                        onClick={() => alert(`${t('mesDemandes.details')} ${demande.reference}`)}
                      >
                        {t('mesDemandes.consulter')}
                      </button>
                      {demande.statut === t('mesDemandes.etapes.telechargement') && demande.lienAutorisation && (
                        <a href={demande.lienAutorisation} download>
                          <button className="btn-telecharger">{t('mesDemandes.download')}</button>
                        </a>
                      )}
                    </td>
                  </tr>
