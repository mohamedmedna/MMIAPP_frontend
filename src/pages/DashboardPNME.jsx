...
function SidebarPNME({ user, onLogout, activeMenu, setActiveMenu }) {
  const links = [
    { key: 'a_traiter', label: 'Dossiers à traiter', icon: <FileDoneOutlined /> },
      ...
      <div className="sidebar-menu">
        {links.map(link => (
          <button 
            ...
            className={`sidebar-link ${activeMenu === link.key ? 'active' : ''}`}
          >
      ...
      <div className="stats-horizontal-container">
        {statItems.map(({ label, value, color }) => (
          <div key={label} className="stat-item">
            ...
            <div className="stat-item-value" style={{ color }}>{value ?? 0}</div>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="reporting-container">
      ...
      <div className="reporting-card">
        <p>Générez un rapport complet de toutes les demandes PNME au format CSV.</p>
        <button 
...
function TableDemandesPNME({ demandes, onVoirDetails, onValider, onRejeter, onDemanderPieces }) {
  return (
    <div className="table-container">
      <table className="demandes-table">
        ...
        <tbody>
          {demandes.length === 0 ? (
            <tr><td colSpan="6" className="no-demandes-row">Aucune demande disponible</td></tr>
          ) : demandes.map(demande => (
            <tr key={demande.id}>
              <td>{demande.reference}</td>
              <td>{new Date(demande.created_at).toLocaleDateString('fr-FR')}</td>
              <td>
                <span className={`statut-badge statut-${demande.statut?.toLowerCase()}`}>
              ...
              <td className="actions-cell">
                <button onClick={() => onVoirDetails(demande)} className="action-btn view">Voir</button>
                <button onClick={() => onValider(demande)} className="action-btn validate">Valider</button>
  ...
  return (
    <div className="popup-overlay" onClick={onClose}>
      <div className="popup-content" onClick={e => e.stopPropagation()}>
        <button onClick={onClose} className="popup-close">&times;</button>
        <h2>Détail de la demande {demande.reference}</h2>
        <p><b>Date:</b> {new Date(demande.created_at).toLocaleDateString('fr-FR')}</p>
        ...
        <h3 style={{marginTop: 24, color: '#229954'}}>Historique des actions</h3>
          ...
          <ul style={{listStyle: 'none', padding: 0, maxHeight: 150, overflowY: 'auto'}}>
            {historique.map(h => (
              <li key={h.id} style={{borderBottom: '1px solid #eee', padding: '8px 0'}}>
                <b>{h.action.replace(/_/g,' ')}</b> par <i>{h.utilisateur || 'Système'}</i>
                <div style={{fontSize: '0.9rem', color: '#666'}}>{h.message}</div>
                <div style={{fontSize: '0.8rem', color: '#999', textAlign: 'right'}}>{new Date(h.date_action).toLocaleString('fr-FR')}</div>
              </li>
            ))}
          </ul>
        )}
...
function DashboardPNME({ user }) {
  ...
  useEffect(() => {
    ...
    const fetchDemandes = async (endpoint, setter) => {
      const res = await fetch(endpoint, { headers: { 'Authorization': `Bearer ${localStorage.getItem('pnme_token')}` } });
    ...
    Promise.all([
      ...
      fetch(endpoints.stats, { headers: { 'Authorization': `Bearer ${localStorage.getItem('pnme_token')}` } }).then(res => res.json())
    ]).then(([, , statsJson]) => {
  ...
  return (
    <div className="dashboard-pnme-page">
      ...
      <div className="dashboard-container">
        <SidebarPNME user={user} onLogout={handleLogout} activeMenu={activeMenu} setActiveMenu={setActiveMenu} />
              ...
              <>
                {activeMenu === 'a_traiter' && (
                  <>
                    ...
                    <TableDemandesPNME demandes={demandesATraiter} onVoirDetails={setPopup} onValider={handleValider} onRejeter={handleRejeter} onDemanderPieces={handleDemanderPieces} />
                  </>
                )}
                 {activeMenu === 'historique' && (
                  <>
                    <h2 className="content-title">Historique des demandes</h2>
                    <TableDemandesPNME demandes={demandesHistorique} onVoirDetails={setPopup} onValider={() => {}} onRejeter={() => {}} onDemanderPieces={() => {}} />
                  </>
                )}
