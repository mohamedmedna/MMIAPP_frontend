...
export default function DashboardDDPI() {
  ...
  const checkTokenValidity = async () => {
    try {
      ...
      const res = await fetch('http://localhost:4000/api/ddpi/verify-token', {
      ...
      console.error('❌ [DDPI] Erreur lors de la vérification du token:', error);
      // Nettoyer tous les tokens possibles
      localStorage.removeItem('adminToken');
      localStorage.removeItem('token');
      localStorage.removeItem('user');
  ...
  const fetchStats = async () => {
    try {
      ...
      const res = await fetch('http://localhost:4000/api/ddpi/stats', {
        headers: { Authorization: `Bearer ${currentToken}` }
      });
      ...
      if (res.status === 401) {
        console.error('❌ [DDPI] Token expiré (mode debug - pas de redirection)');
        // Afficher des données factices pour le test
        setStats([
          { id: 1, label: t('dashboardDDPI.stats.pending'), value: 0, color: '#1890ff' },
          { id: 2, label: t('dashboardDDPI.stats.validated'), value: 0, color: '#52c41a' },
          { id: 3, label: t('dashboardDDPI.stats.rejected'), value: 0, color: '#ff4d4f' },
          { id: 4, label: t('dashboardDDPI.stats.total'), value: 0, color: '#722ed1' }
        ]);
        return;
      }
      ...
      console.error('❌ [DDPI] Erreur lors du chargement des statistiques:', err);
  ...
  const fetchNotifications = async () => {
    try {
      ...
      const res = await fetch('http://localhost:4000/api/ddpi/notifications', {
        headers: { Authorization: `Bearer ${currentToken}` }
      });
      ...
      if (res.status === 401) {
        ...
        setNotifications([
          { id: 1, message: t('dashboardDDPI.notifications.noNotifications'), date: new Date().toISOString(), isNew: false }
        ]);
        return;
      }
      ...
      console.log('✅ [DDPI] Notifications chargées:', data.notifications?.length || 0);
    } catch (err) {
      console.error('❌ [DDPI] Erreur lors du chargement des notifications:', err);
      // TEMPORAIRE: Ne pas afficher d'erreur pour le test
  ...
  const fetchDemandes = async () => {
    ...
    try {
      ...
      const res = await fetch('http://localhost:4000/api/ddpi/demandes', {
        headers: { Authorization: `Bearer ${currentToken}` }
      });
      ...
      notification.error({
        ...
        description: t('dashboardDDPI.messages.validationError') + ': ' + err.message,
        duration: 6
      });
      setDemandes([]);
    } finally {
  ...
  const fetchHistorique = async (demandeId) => {
    setHistLoading(true);
    try {
      console.log(`🔄 [DDPI] Chargement de l'historique pour la demande ${demandeId}`);
      const res = await fetch(`http://localhost:4000/api/ddpi/demandes/${demandeId}/historique`, {
      ...
      console.log('✅ [DDPI] Historique chargé:', data.historique?.length || 0, 'entrées');
    } catch (err) {
      console.error('❌ [DDPI] Erreur lors du chargement de l\'historique:', err);
      notification.error({
        message: t('dashboardDDPI.messages.error'),
  ...
  const submitCommentaire = async () => {
    ...
    try {
      console.log(`🔄 [DDPI] Ajout de commentaire pour la demande ${selectedDemande.id}`);
      
      const res = await fetch(`http://localhost:4000/api/ddpi/demandes/${selectedDemande.id}/commentaire`, {
        method: 'POST',
        headers: { 
      ...
      if (res.ok) {
        ...
        console.log('✅ [DDPI] Commentaire ajouté pour:', selectedDemande.reference);
  ...
  const handleValider = (demande) => {
    Modal.confirm({
      ...
      content: (
        <div>
          <p style={{ marginBottom: '16px' }}>
            <strong>{t('dashboardDDPI.confirm.demand')}:</strong> {demande.reference}
          </p>
          <p style={{ marginBottom: '16px' }}>
            <strong>{t('dashboardDDPI.confirm.applicant')}:</strong> {demande.demandeur}
          </p>
          <p style={{ marginBottom: '16px' }}>
      ...
      onOk: async () => {
        try {
          console.log(`🔄 [DDPI] Validation de la demande ${demande.id} - ${demande.reference}`);
          
          const res = await fetch(`http://localhost:4000/api/ddpi/demandes/${demande.id}/valider`, {
            method: 'POST',
            headers: { 
              'Content-Type': 'application/json',
              Authorization: `Bearer ${token}` 
            },
            body: JSON.stringify({ commentaire: t('dashboardDDPI.validation.comment') })
          });
          
          console.log('📊 [DDPI] Réponse validation:', res.status, res.statusText);
          ...
          notification.success({ 
            message: t('dashboardDDPI.messages.success'), 
            description: data.message || t('dashboardDDPI.messages.demandValidated', { reference: demande.reference }),
            duration: 4
          });
          ...
          notification.error({ 
            message: t('dashboardDDPI.messages.error'), 
            description: err.message || t('dashboardDDPI.messages.validationError'),
            duration: 6
          });
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        ...
        try {
          ...
          const res = await fetch(`http://localhost:4000/api/ddpi/demandes/${demande.id}/retour`, {
            method: 'POST',
            headers: { 
              'Content-Type': 'application/json', 
              Authorization: `Bearer ${token}` 
          ...
          if (res.ok) {
            notification.success({ 
              message: t('dashboardDDPI.messages.success'), 
              description: data.message || t('dashboardDDPI.messages.demandReturned'),
              duration: 4
            });
  ...
  const handleTransmettre = (demande) => {
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          console.log(`🔄 [DDPI] Transmission de la demande ${demande.id} - ${demande.reference} vers DGI`);
          
          const res = await fetch(`http://localhost:4000/api/ddpi/demandes/${demande.id}/transmettre`, {
            method: 'POST',
            headers: { 
              'Content-Type': 'application/json',
              Authorization: `Bearer ${token}` 
          ...
          console.log('📊 [DDPI] Réponse transmission:', res.status, res.statusText);
          ...
          notification.success({ 
            message: t('dashboardDDPI.messages.success'), 
            description: data.message || t('dashboardDDPI.messages.demandTransmitted', { reference: demande.reference }),
            duration: 4
          });
          ...
          notification.error({ 
            message: t('dashboardDDPI.messages.error'), 
            description: err.message || t('dashboardDDPI.messages.transmitError'),
            duration: 6
          });
          return Promise.reject();
        }
  ...
  const columns = [
    {
      ...
      dataIndex: 'reference',
      key: 'reference',
      render: ref => ref || <Tag color="default">{t('dashboardDDPI.demands.none')}</Tag>,
    },
    {
    ...
    },
    {
      ...
      render: (_, demande) => (
        <>
          <Button icon={<FileTextOutlined />} onClick={() => handleConsulter(demande)} style={{ marginRight: 8 }}>{t('dashboardDDPI.actions.view')}</Button>
          <Button icon={<CommentOutlined />} onClick={() => handleCommenter(demande)} style={{ marginRight: 8 }}>{t('dashboardDDPI.actions.comment')}</Button>
          {demande.statut === 'TRANSMISE_A_DDPI' && (
            <>
              <Button icon={<CheckCircleOutlined />} type="primary" onClick={() => handleValider(demande)} style={{ marginRight: 8 }}>{t('dashboardDDPI.actions.validate')}</Button>
              <Button icon={<CloseCircleOutlined />} danger onClick={() => handleRetourner(demande)} style={{ marginRight: 8 }}>{t('dashboardDDPI.actions.return')}</Button>
            </>
          )}
          {demande.statut === 'VALIDEE_DDPI' && (
            <Button icon={<SendOutlined />} type="primary" onClick={() => handleTransmettre(demande)}>{t('dashboardDDPI.actions.transmitDGI')}</Button>
          )}
        </>
      )
    }
      ...
      <div className="dashboard-ddpi-container">
        ...
        <div className="left-panel">
          ...
          <nav className="sidebar-nav">
            ...
            <button
              className={`nav-item ${activeSidebarLink === 'settings' ? 'active' : ''}`}
        ...
        <div className="main-section">
          ...
          {activeSidebarLink === 'demandes' && (
            <>
              ...
              {loading ? <Spin /> : <Table dataSource={demandes} columns={columns} rowKey="id" pagination={{ pageSize: 10 }} />}
            </>
          )}
          ...
          {activeSidebarLink === 'notifications' && (
            <>
              ...
              <Table
                ...
                columns={[
                  { title: t('dashboardDDPI.notifications.message'), dataIndex: 'message', key: 'message' },
                  { title: t('dashboardDDPI.notifications.date'), dataIndex: 'date', key: 'date', render: d => new Date(d).toLocaleString() },
                  { title: t('dashboardDDPI.notifications.status'), dataIndex: 'isNew', key: 'isNew', render: isNew => isNew ? <Tag color="red">{t('dashboardDDPI.notifications.new')}</Tag> : <Tag color="green">{t('dashboardDDPI.notifications.read')}</Tag> }
                ]}
                rowKey="id"
          ...
          >
            {selectedDemande && (
              <div>
                <h3>{t('dashboardDDPI.demandDetails.reference')}: {selectedDemande.reference}</h3>
                <p><strong>{t('dashboardDDPI.demandDetails.applicant')}:</strong> {selectedDemande.demandeur}</p>
                <p><strong>{t('dashboardDDPI.demandDetails.type')}:</strong> {selectedDemande.type}</p>
                <p><strong>{t('dashboardDDPI.demandDetails.date')}:</strong> {new Date(selectedDemande.created_at).toLocaleString()}</p>
                <p><strong>{t('dashboardDDPI.demandDetails.status')}:</strong> {selectedDemande.statut}</p>
                ...
                {histLoading ? <Spin /> : (
                  <Table
                    ...
                    columns={[
                      { title: t('dashboardDDPI.history.action'), dataIndex: 'action', key: 'action' },
                      { title: t('dashboardDDPI.history.message'), dataIndex: 'message', key: 'message' },
                      { title: t('dashboardDDPI.history.date'), dataIndex: 'date_action', key: 'date_action', render: d => new Date(d).toLocaleString() },
                      { title: t('dashboardDDPI.history.previousStatus'), dataIndex: 'statut_precedent', key: 'statut_precedent' },
                      { title: t('dashboardDDPI.history.newStatus'), dataIndex: 'nouveau_statut', key: 'nouveau_statut' }
                    ]}
                    rowKey="id"
                    pagination={false}
                    size="small"
