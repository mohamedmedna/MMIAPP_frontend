...
import { Layout, Menu, Table, Button, Modal, Tag, Spin, Input, notification, Statistic, Row, Col, Card, Descriptions, Steps, Form, Select, Space, Divider } from 'antd';
import {
  DashboardOutlined,
  FileTextOutlined,
  BellOutlined,
...
export default function DashboardComite() {
  ...
  const fetchStats = async () => {
    setLoading(true);
    try {
      const res = await fetch('http://localhost:4000/api/commission/stats', {
  ...
  const fetchNotifications = async () => {
    try {
      const res = await fetch('http://localhost:4000/api/commission/notifications', {
        headers: { Authorization: `Bearer ${token}` }
      });
      const data = await res.json();
  ...
  const fetchDossiers = async () => {
    setLoading(true);
    try {
      const res = await fetch('http://localhost:4000/api/commission/dossiers', {
        headers: { Authorization: `Bearer ${token}` }
      });
      const data = await res.json();
  ...
  const fetchHistorique = async (id) => {
    setHistLoading(true);
    try {
      const res = await fetch(`http://localhost:4000/api/commission/dossiers/${id}/historique`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      const data = await res.json();
      setHistorique(data.historique || []);
    } catch (err) {
      notification.error({ message: 'Erreur', description: 'Impossible de charger l\'historique' });
    } finally {
  ...
  const fetchCriteresTechniques = async (id) => {
    ...
    try {
      const res = await fetch(`http://localhost:4000/api/commission/dossiers/${id}/criteres`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      const data = await res.json();
      setCriteresTechniques(data.criteres || {});
    } catch (err) {
      notification.error({ message: 'Erreur', description: 'Impossible de charger les critères techniques' });
    } finally {
  ...
  const handleAvis = async (type_avis) => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:4000/api/commission/dossiers/${selectedDossier.id}/avis`, {
        method: 'POST',
      ...
      if (res.ok) {
        ...
        notification.success({ 
          message: 'Succès', 
          description: `Avis enregistré. ${result.prochaineEtape !== 'REJETE' ? `Transmission à ${result.prochaineEtape} en cours.` : 'Demande rejetée.'}` 
        });
        ...
        notification.error({ message: 'Erreur', description: 'Erreur lors de l\'enregistrement de l\'avis' });
      }
    } catch (error) {
      if (error.errorFields) {
        notification.error({ message: 'Erreur', description: 'Veuillez remplir tous les champs requis' });
      } else {
      notification.error({ message: 'Erreur', description: 'Erreur réseau ou serveur' });
      }
    }
  };
  ...
  const getStatutText = (statut) => {
    if (statut.includes('EN_ATTENTE_AVIS_COMMISSION')) return 'En attente d\'avis';
    if (statut.includes('AVIS_FAVORABLE_COMMISSION')) return 'Avis favorable';
    if (statut.includes('AVIS_DEFAVORABLE_COMMISSION')) return 'Avis défavorable';
    ...
    return Object.entries(criteresTechniques).map(([key, critere]) => (
      <Card key={key} size="small" style={{ marginBottom: 8 }}>
        <Descriptions column={1} size="small">
          ...
          <Descriptions.Item label="Valeur actuelle">{critere.valeur}</Descriptions.Item>
          <Descriptions.Item label="Statut">
            <Tag color={critere.statut === 'Conforme' ? 'green' : 'orange'}>{critere.statut}</Tag>
          </Descriptions.Item>
          <Descriptions.Item label="Commentaire">{critere.commentaire}</Descriptions.Item>
        </Descriptions>
      </Card>
    ));
  };
      ...
      <div className="dashboard-comite-container">
        ...
        <div className="left-panel">
          ...
          <nav className="sidebar-nav">
            <button
              className={`nav-item ${activeMenuKey === 'dashboard' ? 'active' : ''}`}
            ...
            <button
              className={`nav-item ${activeMenuKey === 'dossiers' ? 'active' : ''}`}
            ...
            <button
              className={`nav-item ${activeMenuKey === 'notifications' ? 'active' : ''}`}
            ...
            <button
              className={`nav-item ${activeMenuKey === 'parametres' ? 'active' : ''}`}
        ...
        <div className="main-section">
          <div className="dashboard-header">
            <h1 className="dashboard-title">🏢 Tableau de bord Comité Technique</h1>
          </div>
              ...
              <Table
                ...
                columns={[
                  ...
                  { title: 'Date', dataIndex: 'date', key: 'date', render: d => new Date(d).toLocaleString() },
                  {
                    ...
                    dataIndex: 'isNew',
                    key: 'isNew',
                    render: isNew => isNew ? <Tag color="red">Nouveau</Tag> : <Tag color="green">Lu</Tag>
                  }
                ]}
                rowKey="id"
                pagination={{ pageSize: 10 }}
      ...
      >
        {selectedDossier && (
          <div>
            ...
            {currentStep === 0 && (
              <div>
                <Card title="Informations du dossier" style={{ marginBottom: 16 }}>
                  <Descriptions column={2}>
                    <Descriptions.Item label="Référence">{selectedDossier.reference}</Descriptions.Item>
                    <Descriptions.Item label="Type">{selectedDossier.type}</Descriptions.Item>
                    <Descriptions.Item label="Demandeur">{selectedDossier.demandeur}</Descriptions.Item>
                    <Descriptions.Item label="Date de création">
                      {new Date(selectedDossier.created_at).toLocaleDateString()}
                    </Descriptions.Item>
                    <Descriptions.Item label="Statut">
                ...
                <Card title="Données de la demande" style={{ marginBottom: 16 }}>
            ...
            {currentStep === 1 && (
              <div>
                <Card title="Critères techniques à vérifier" style={{ marginBottom: 16 }}>
            ...
            {currentStep === 2 && (
              <div>
                <Card title="Émission d'avis et transmission" style={{ marginBottom: 16 }}>
                  <Form form={form} layout="vertical">
                    <Form.Item
                      name="criteres_techniques"
                      label="Évaluation des critères techniques"
                    >
                      <Select mode="multiple" placeholder="Sélectionner les critères conformes">
                        {Object.entries(criteresTechniques).map(([key, critere]) => (
