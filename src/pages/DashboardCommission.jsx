...
import { Layout, Menu, Table, Button, Modal, Tag, Spin, Input, notification, Tabs, Statistic, Row, Col, Card, Typography, Space, Divider } from 'antd';
import {
  DashboardOutlined,
  FileTextOutlined,
  BellOutlined,
...
export default function DashboardCommission() {
  ...
  const fetchDossiersTraites = async () => {
    ...
    try {
      const res = await fetch('http://localhost:4000/api/commission/dossiers-traites', {
        headers: { Authorization: `Bearer ${token}` }
      });
      const data = await res.json();
  ...
  const handleAvis = async (type_avis) => {
    ...
    setAvisLoading(true);
    try {
      console.log('📋 [frontend] Envoi avis:', { dossierId: selectedDossier.id, type_avis, observations: avis });
      ...
      if (res.ok && data.success) {
        ...
        notification.error({ 
          message: t('dashboardCommission.messages.error'), 
          description: data.error || data.details || t('dashboardCommission.avis.error') 
        });
      }
    } catch (error) {
  ...
  const getStatusColor = (statut) => {
    ...
    if (statut.includes('VALIDE') || statut.includes('FAVORABLE')) return 'green';
    if (statut.includes('RESERVE')) return 'orange';
    if (statut.includes('REJETE') || statut.includes('DEFAVORABLE')) return 'red';
    return 'default';
  };
  ...
  return (
    <>
      ...
      <div className="dashboard-commission-container">
        ...
        <div className="left-panel">
          ...
          <nav className="sidebar-nav">
            ...
            <button
              className={`nav-item ${activeMenuKey === 'dossiers-traites' ? 'active' : ''}`}
        ...
        <div className="main-section">
          <div className="dashboard-header">
            <h1 className="dashboard-title">🏢 {t('dashboardCommission.title')}</h1>
          </div>
            ...
            <>
              ...
              {loading || !stats ? (
                <div style={{ textAlign: 'center', padding: '50px' }}>
                  <Spin size="large" tip={t('dashboardCommission.messages.loadingStats')} />
                </div>
              ) : (
                <>
                  <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
                    {stats.map(stat => (
                      <Col xs={24} sm={12} md={6} key={stat.id}>
                        <Card>
                          <Statistic 
                            title={t(`dashboardCommission.statistics.${stat.id === '1' ? 'aExaminer' : stat.id === '2' ? 'valides' : stat.id === '3' ? 'reserves' : 'rejetes'}`)}
                            value={stat.value} 
                            valueStyle={{ color: stat.color }}
                            prefix={
                              stat.id === '1' ? <ClockCircleOutlined /> :
                              stat.id === '2' ? <CheckCircleOutlined /> :
                              stat.id === '3' ? <ExclamationCircleOutlined /> :
                              <CloseCircleOutlined />
                            }
                          />
                        </Card>
                  ...
                  <Card title={t('dashboardCommission.statistics.title')} style={{ textAlign: 'center' }}>
          ...
          {activeMenuKey === 'dossiers-traites' && (
            <>
              <Title level={2}>{t('dashboardCommission.historique.title')}</Title>
              <Table
                ...
                pagination={{ 
                  ...
                  showSizeChanger: true,
                  showQuickJumper: true,
                  showTotal: (total, range) => `${range[0]}-${range[1]} sur ${total} dossiers dans l'historique`
                }}
                scroll={{ x: 800 }}
                locale={{
          ...
          {activeMenuKey === 'notifications' && (
            <>
              <Title level={2}>{t('dashboardCommission.notifications.title')}</Title>
              <Table
                ...
                columns={[
                  ...
                  },
                  {
                    ...
                    width: 100,
                    render: isNew => isNew ? 
                      <Tag color="red">{t('dashboardCommission.notifications.nouveau')}</Tag> : 
                      <Tag color="green">{t('dashboardCommission.notifications.lu')}</Tag>
                  }
                ]}
                rowKey="id"
                pagination={{ pageSize: 10 }}
                locale={{
                  emptyText: t('dashboardCommission.notifications.noNotifications')
                }}
              />
            </>
          )}
          ...
          {activeMenuKey === 'parametres' && (
            <div>
              <Title level={2}>{t('dashboardCommission.sidebar.parametres')}</Title>
              <Card>
      ...
      <Modal
        title={`${t('dashboardCommission.modal.detailTitle')} ${selectedDossier?.reference || ''}`}
        open={showModal}
          ...
          <Space direction="vertical" style={{ width: '100%' }} size="large">
            ...
            <TextArea
              ...
              onChange={e => setAvis(e.target.value)}
                placeholder={t('dashboardCommission.avis.observationsPlaceholder')}
                style={{ marginTop: 8 }}
            />
            </div>
            ...
            <div>
              <Title level={4}>{t('dashboardCommission.modal.historique')}</Title>
              {histLoading ? (
                <Spin />
              ) : (
              <Table
