...
export default function DashboardDRMNE() {
  ...
  const fetchDemandes = async (statut = "TOUTES") => {
    ...
    try {
      const response = await fetch(
        `${API_BASE}/api/drmne/demandes?statut=${statut}`,
        {
          headers: {
  ...
  const fetchStats = async () => {
    try {
      const response = await fetch(
        `${API_BASE}/api/drmne/demandes?statut=TOUTES`,
        {
          headers: {
            Authorization: `Bearer ${getToken()}`,
          },
      ...
      if (response.ok) {
        ...
        const enCours =
          data.demandes?.filter((d) => d.statut === "EN_COURS_TRAITEMENT")
            .length || 0;
        const piecesManquantes =
          data.demandes?.filter((d) => d.statut === "PIECES_MANQUANTES")
            .length || 0;
        const validees =
          data.demandes?.filter((d) => d.statut === "EN_ATTENTE_SIGNATURE")
            .length || 0;
  ...
  const handleDemanderComplement = async () => {
    ...
    try {
      const response = await fetch(
        `${API_BASE}/api/drmne/demandes/${demandeAction.id}/demander-complement`,
        {
          method: "POST",
          headers: {
  ...
  const handleRejeter = async () => {
    ...
    try {
      const response = await fetch(
        `${API_BASE}/api/drmne/demandes/${demandeAction.id}/rejeter`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleTransmettre = async () => {
    try {
      const response = await fetch(
        `${API_BASE}/api/drmne/demandes/${demandeAction.id}/transmettre`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (response.ok) {
        message.success(
          `Demande transmise vers ${cibleTransmission} avec succès`
        );
  ...
  const handleValider = async () => {
    try {
      const response = await fetch(
        `${API_BASE}/api/drmne/demandes/${demandeAction.id}/valider`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleVoirHistorique = async (demande) => {
    try {
      const response = await fetch(
        `${API_BASE}/api/drmne/demandes/${demande.id}/historique`,
        {
          headers: {
            Authorization: `Bearer ${getToken()}`,
          },
