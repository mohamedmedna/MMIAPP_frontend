  ...
  const onFinish = async (values) => {
    ...
    try {
      ...
      if (response.ok && data.token && data.user && data.user.role_id === 9) {
        ...
        // ✅ SPA navigation so basename is applied
        navigate("/dashboard-ministre");
        ...
        <div className="login-antd-form-container">
          ...
          >
            <Form.Item
              ...
              rules={[
                ...
                {
                  type: "email",
                  message: t("loginministre.error_email_invalid"),
                },
              ]}
            >
              <Input
                prefix={<UserOutlined />}
                placeholder={t("loginministre.email")}
              />
            </Form.Item>
            ...
            <Form.Item
              ...
              name="password"
              rules={[
                { required: true, message: t("loginministre.error_password") },
              ]}
            >
              <Input.Password
                prefix={<LockOutlined />}
                placeholder={t("loginministre.password")}
              />
            </Form.Item>
          ...
          <div className="login-antd-links">
            <Link to="/forgot-password" className="login-antd-link">
              {t("loginministre.forgot_password")}
            </Link>
          </div>

          <Link to="/" className="login-antd-back-home">
            {t("loginministre.back_home")}
          </Link>
        </div>
      </div>
