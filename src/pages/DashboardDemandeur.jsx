...
export default function DashboardDemandeur({ user, logout }) {
  ...
  const checkForAccuseUpdates = () => {
    ...
    console.log(`🔍 Vérification des mises à jour d'accusé. Dernière vérification: ${new Date(lastAccuseCheck).toLocaleTimeString()}, Dernière mise à jour: ${lastUpdateTime ? new Date(lastUpdateTime).toLocaleTimeString() : 'jamais'}`);
    
    if (lastUpdateTime > lastAccuseCheck) {
      console.log('🔄 Nouvelle mise à jour d\'accusé détectée, rafraîchissement automatique...');
    ...
    try {
      ...
      if (demandes.length > 0) {
        const newAccuses = data.filter(newDemande => {
          ...
          return oldDemande && !oldDemande.fichier_accuse && newDemande.fichier_accuse;
        });
        
        if (newAccuses.length > 0) {
          console.log('🎉 Nouveaux accusés de réception détectés:', newAccuses.map(d => d.reference));
  ...
  const handleDownloadAutorisation = async (reference) => {
    try {
      console.log(`📄 Téléchargement de l'autorisation pour la référence ${reference}`);
      ...
      console.error('❌ Erreur lors du téléchargement de l\'autorisation:', error);
      alert('Erreur lors du téléchargement de l\'autorisation. Vérifiez que vous êtes connecté et que l\'autorisation est disponible.');
    }
  };
      ...
      <div className="dashboard-demandeur-container">
        ...
        <div className="main-section">
          <div className="dashboard-header">
            <h1 className="dashboard-title">🏠 Bienvenue {user?.prenom_responsable || 'Utilisateur'} {user?.nom_responsable || ''}</h1>
          </div>
          
          <div className="dashboard-content">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              <h2>Bienvenue {user?.prenom_responsable || 'Utilisateur'} {user?.nom_responsable || ''}</h2>
              <button 
              ...
              >
                ...
                {refreshing ? t('dashboardDemandeur.messages.loading') : t('dashboardDemandeur.actions.nouvelleDemande')}
              </button>
            </div>
            ...
            <div className="dashboard-progress-bar">
              <div className="progress-item">
                <span className="progress-label">{t('dashboardDemandeur.statistiques.totalDemandes')}</span>
                <span className="progress-value">{stats.total}</span>
              </div>
              <div className="progress-separator" />
              <div className="progress-item">
                <span className="progress-label">{t('dashboardDemandeur.statistiques.enCours')}</span>
                <span className="progress-value">{stats.enCours}</span>
              </div>
              <div className="progress-separator" />
              <div className="progress-item">
                <span className="progress-label">{t('dashboardDemandeur.statistiques.validees')}</span>
                <span className="progress-value">{stats.approuvees}</span>
              </div>
              <div className="progress-separator" />
              <div className="progress-item">
                <span className="progress-label">{t('dashboardDemandeur.statistiques.rejetees')}</span>
            ...
            <div className="dashboard-actions">
              <Link to="/nouvelle-demande" className="dashboard-btn dashboard-btn-primary">
                <PlusCircleOutlined /> {t('dashboardDemandeur.actions.nouvelleDemande')}
              </Link>
              <Link to="/suivi-demandes" className="dashboard-btn dashboard-btn-secondary">
                <i className="fa fa-folder-open" style={{ marginRight: 8 }}></i> Suivi des Demandes
              </Link>
              <Link to="/notifications" className="dashboard-btn dashboard-btn-info">
                <i className="fa fa-bell" style={{ marginRight: 8 }}></i> Notifications
              </Link>
              <Link to="/archive" className="dashboard-btn dashboard-btn-success">
                <i className="fa fa-archive" style={{ marginRight: 8 }}></i> Archive
              </Link>
            </div>
            ...
            <div className="dashboard-section suivi-demandes-section">
              <h3 className="section-title">
                <i className="fa fa-road" style={{ marginRight: '8px', color: '#1e6a8e' }}></i>
              ...
              {loading ? (
                ...
                <div className="no-demandes">
                  <i className="fa fa-inbox" style={{ fontSize: '3em', color: '#ccc', marginBottom: '16px' }}></i>
                    ...
                    <div key={demande.id} className="demande-progression-item">
                      <div className="demande-progression-header">
                        <div className="demande-info">
                          <span className="demande-reference">{demande.reference}</span>
                          <span className="demande-type">{demande.type}</span>
                        </div>
                        <span className={`demande-statut ${demande.statut.toLowerCase().replace(/_/g, '-')}`}>
                      ...
                      <div className="demande-progression-bar">
                        <div className="progression-bar">
                          <div 
                            ...
                            style={{ width: getProgressPercentage(demande.statut) + '%' }}
                          ></div>
                        </div>
                      ...
                      <div className="demande-etapes">
                        <div className="etape-item">
                          <span className={`etape-icon ${demande.statut === 'DEPOSEE' ? 'completed' : ''}`}>
                        ...
                        <div className="etape-item">
                          <span className={`etape-icon ${['RECEPTIONNEE', 'TRANSMISE_AU_SG', 'TRANSMISE_AU_DGI', 'EN_COURS_DGI', 'VALIDEE_DGI', 'TRANSMISE_AU_MINISTRE', 'AUTORISATION_SIGNEE'].includes(demande.statut) ? 'completed' : ''}`}>
                        ...
                        <div className="etape-item">
                          <span className={`etape-icon ${['TRANSMISE_AU_SG', 'TRANSMISE_AU_DGI', 'EN_COURS_DGI', 'VALIDEE_DGI', 'TRANSMISE_AU_MINISTRE', 'AUTORISATION_SIGNEE'].includes(demande.statut) ? 'completed' : ''}`}>
                        ...
                        <div className="etape-item">
                          <span className={`etape-icon ${['EN_COURS_DGI', 'VALIDEE_DGI', 'TRANSMISE_AU_MINISTRE', 'AUTORISATION_SIGNEE'].includes(demande.statut) ? 'completed' : ''}`}>
                        ...
                        <div className="etape-item">
                          <span className={`etape-icon ${['VALIDEE_DGI', 'TRANSMISE_AU_MINISTRE', 'AUTORISATION_SIGNEE'].includes(demande.statut) ? 'completed' : ''}`}>
                        ...
                        <div className="etape-item">
                          <span className={`etape-icon ${['AUTORISATION_SIGNEE'].includes(demande.statut) ? 'completed' : ''}`}>
                      ...
                      <div className="demande-actions">
                        ...
                        {demande.fichier_accuse && (
                          ...
                          >
                            <i className="fa fa-download"></i> Télécharger Accusé
                          </button>
                        )}
                        {(demande.statut === 'AUTORISATION_SIGNEE' || demande.statut === 'CLOTUREE') && demande.autorisation_pdf && (
                          ...
                          >
                            <i className="fa fa-certificate"></i> Télécharger Autorisation
                          </button>
                        )}
                      </div>
                    </div>
