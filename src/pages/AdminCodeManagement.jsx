...
import { Form, Input, Button, Alert, Typography, Card, Space, Divider } from 'antd';
...
export default function AdminCodeManagement({ user, logout }) {
  ...
  const loadCurrentCode = async () => {
    try {
      ...
      const response = await fetch('http://localhost:4000/api/admin/get-access-code', {
        headers: {
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('http://localhost:4000/api/admin/update-access-code', {
        method: 'POST',
        headers: {
  ...
  const validateCode = (_, value) => {
    ...
    if (value.length !== 8) {
      return Promise.reject(new Error('Le code doit contenir exactement 8 caractères !'));
    }
    ...
    if (!hasLetter || !hasNumber || !hasSpecial) {
      return Promise.reject(new Error('Le code doit contenir au moins une lettre, un chiffre et un caractère spécial !'));
    }
        ...
        <Card className="update-card">
          ...
          <div className="requirements">
            ...
            <div style={{ marginTop: '20px', padding: '15px', backgroundColor: '#fff3cd', border: '1px solid #ffeaa7', borderRadius: '8px' }}>
              <Text strong style={{ color: '#856404' }}>🔒 Note de Sécurité :</Text>
              <br />
              <Text style={{ color: '#856404', fontSize: '0.9rem' }}>
                Le code d'accès est confidentiel. Ne le partagez qu'avec les personnes autorisées.
                Changez le code par défaut dès la première utilisation.
              </Text>
            </div>
          </div>
        </Card>
