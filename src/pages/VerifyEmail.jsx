...
const VerifyEmail = () => {
    ...
    const verifyEmail = async () => {
      try {
        ...
        if (response.ok && data.success) {
          ...
          setStatus('error');
          setMessage(data.error || 'Erreur lors de l\'activation de votre compte.');
        }
      } catch (error) {
        console.error('Erreur lors de la vérification:', error);
        setStatus('error');
        setMessage('Une erreur s\'est produite lors de l\'activation de votre compte.');
      }
    };
  ...
  const messageStyle = {
    ...
    marginBottom: '20px',
    lineHeight: '1.5',
    color: status === 'success' ? '#2e7d32' : status === 'error' ? '#c62828' : '#1e6a8e',
  };
  ...
  return (
    <div style={containerStyle}>
      <div style={cardStyle}>
        ...
        {status === 'success' && (
          <>
            <div style={{ fontSize: '3rem', color: '#2e7d32', marginBottom: '20px' }}>
              ✓
            </div>
            <p style={messageStyle}>
              {message}
        ...
        {status === 'error' && (
          <>
            <div style={{ fontSize: '3rem', color: '#c62828', marginBottom: '20px' }}>
              ✗
            </div>
            <p style={messageStyle}>
              {message}
