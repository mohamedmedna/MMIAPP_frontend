  ...
  const onFinish = async (values) => {
    ...
    try {
      ...
      if (response.ok && data.token && data.user && data.user.role_id === 2) {
        // Stocker le token dans les deux emplacements (compat)
        localStorage.setItem("token", data.token);
        localStorage.setItem("adminToken", data.token);
        localStorage.setItem("user", JSON.stringify(data.user));

        navigate("/dashboard-secretaire"); // SPA nav (respecte basename)
      ...
      <div className="login-container">
        <div className="login-intro">
          ...
          <Title level={3} style={{ color: "#229954" }}>
            {t("loginSecretaireCentral.loginSpace")}
          </Title>
        </div>
        ...
        <div className="login-antd-form-container">
          ...
          >
            <Form.Item
              ...
              rules={[
                {
                  required: true,
                  message: t("loginSecretaireCentral.emailRequired"),
                },
                {
                  type: "email",
                  message: t("loginSecretaireCentral.emailInvalid"),
                },
              ]}
            >
              <Input
                prefix={<UserOutlined />}
                placeholder={t("loginSecretaireCentral.emailPlaceholder")}
              />
            </Form.Item>
            ...
            <Form.Item
              ...
              rules={[
                {
                  required: true,
                  message: t("loginSecretaireCentral.passwordRequired"),
                },
              ]}
            >
              <Input.Password
                prefix={<LockOutlined />}
                placeholder={t("loginSecretaireCentral.passwordPlaceholder")}
              />
            </Form.Item>
          ...
          <div className="login-antd-links">
            <Link to="/forgot-password" className="login-antd-link">
              {t("loginSecretaireCentral.forgotPassword")}
            </Link>
          </div>

          <Link to="/" className="login-antd-back-home">
            ← {t("loginSecretaire.backHome")}
          </Link>
        </div>
      </div>
