...
export default function LoginSecretaireGeneral() {
  ...
  const onFinish = async (values) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`${baseUrl}/api/login/secretaire-general`, {
        method: "POST",
      ...
      <div className="login-container">
        <div className="login-intro">
          ...
          <Title level={3} style={{ color: "#229954" }}>
            {t("loginSecretaireGeneral.loginSpace")}
          </Title>
        </div>
        ...
        <div className="login-antd-form-container">
          ...
          >
            <Form.Item
              ...
              rules={[
                {
                  required: true,
                  message: t("loginSecretaireGeneral.emailRequired"),
                },
                {
                  type: "email",
                  message: t("loginSecretaireGeneral.emailInvalid"),
                },
              ]}
            >
              <Input
                prefix={<UserOutlined />}
                placeholder={t("loginSecretaireGeneral.emailPlaceholder")}
              />
            </Form.Item>
            ...
            <Form.Item
              ...
              rules={[
                {
                  required: true,
                  message: t("loginSecretaireGeneral.passwordRequired"),
                },
              ]}
            >
              <Input.Password
                prefix={<LockOutlined />}
                placeholder={t("loginSecretaireGeneral.passwordPlaceholder")}
              />
            </Form.Item>
          ...
          <div className="login-antd-links">
            <Link to="/forgot-password" className="login-antd-link">
              {t("loginSecretaireGeneral.forgotPassword")}
            </Link>
          </div>

          <Link to="/" className="login-antd-back-home">
            ← {t("loginSecretaireGeneral.backHome")}
          </Link>
        </div>
      </div>
